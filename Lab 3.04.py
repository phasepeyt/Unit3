...
Aliasing
--------
1.  Will updating b affect a? Explain why or why not? no, it just sets b to a
a = [1, 2, 4]
b = a
...
def multiply_num(num):
    if num == my_num:
        (num) * input("what would u like to multiply by? ")
    print(num)
...
def add2_and_multiply(num):
    if num == my_num:
        num += 2 + (num) * int(input("what would u like to multiply by? "))
    print(num)

add2_and_multiply(my_num)